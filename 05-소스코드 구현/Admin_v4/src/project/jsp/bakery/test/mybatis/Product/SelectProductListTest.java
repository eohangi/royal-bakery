...
public class SelectProductListTest {
	...
	public static void main(String[] args) {
		...
		item.setProClassify("b");

		try {
			// ProductMapper.selectProductList 기능을 호출한다.
			// 두 번째 파라미터는 조회 조건시에 사용될 파라미터 --> Beans객체
			// 조회 결과가 다중행을 리턴하기 때문에 List형태로 리턴된다.
			list = sqlSession.selectList("ProductMapper.selectProductList", item);
		} catch (Exception e) {
